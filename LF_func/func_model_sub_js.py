...
def _getCostVolume_(inputs):
    ...
    cost_volume = K.reshape(cost_volume,
                            (shape[0], 17, shape[1], shape[2], 4 * 81))
    return cost_volume
...
def define_SubFocal(sz_input, sz_input2, view_n, learning_rate):
    """ 81 inputs"""
    input_list = []
    ...
    cost = Lambda(lambda x: K.permute_dimensions(K.squeeze(x, -1),
                                                 (0, 2, 3, 1)))(cost)
    pred_w = Activation('softmax')(cost)

    pred = Lambda(disparityregression)(pred_w)

    model = Model(inputs=input_list, outputs=[pred, pred_w])
    ...
    model.compile(optimizer=opt, loss=[None, uarl_loss])
...
def uarl_loss(y_true, y_pred):
    # y_true [-4,4] -> [0, 8]
    y_true_label = y_true
    y_pred_label = disparityregression(y_pred)

    y_true = tf.reshape(y_true, (-1, ))
    y_pred = tf.reshape(y_pred, (-1, 17))

    y_true = tf.math.add(y_true, 4.)
    disl = y_true // 0.5
    disr = disl + 1
    ...
    disl_one_hot = tf.one_hot(tf.cast(disl, tf.int32), 17)
    disr_one_hot = tf.one_hot(tf.cast(disr, tf.int32), 17)

    y_pred = tf.cast(y_pred, tf.float32)
    y_true = tf.cast(y_true, tf.float32)
    disl_one_hot = tf.cast(disl_one_hot, tf.float32)
    disr_one_hot = tf.cast(disr_one_hot, tf.float32)
    wl = tf.cast(wl, tf.float32)
    wr = tf.cast(wr, tf.float32)
    # JS divergence based on KL divergence
    y_target = disl_one_hot * tf.reshape(wl,(-1, 1)) + \
        disr_one_hot * tf.reshape(wr, (-1, 1))
    y_middle = 0.5 * (y_target + y_pred)
    # beta stands the exp weights
    beta = 0.1
    uncertainty_map = 0.5 * (tf.keras.losses.kl_divergence(y_pred, y_middle) +
                             tf.keras.losses.kl_divergence(y_target, y_middle))
    uncertainty_map = K.clip(uncertainty_map, K.epsilon(), 1)
    mae_loss = math_ops.abs(
        tf.reshape(y_true_label, (-1, )) - tf.reshape(y_pred_label, (-1, )))
    loss = uncertainty_map**beta * mae_loss
    return K.mean(loss)
...
if __name__ == '__main__':
    ...
    T1 = time.time()
    model = define_SubFocal(input_size, input_size, AngualrViews, 0.001)
    T2 = time.time()
